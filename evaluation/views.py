...
def eva_index2(request): 
    ...
    ooo = StandaredChart.objects.values('oil_name', 'key').order_by('oil_name').distinct()
    
    print(ooo)    
    for o in ooo:
        aaa = StdOils.objects.filter(name = o.get('oil_name'))
        if not aaa.exists():
            StdOils.objects.create(name = o.get('oil_name'))
